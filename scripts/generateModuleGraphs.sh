...
#
# Credits goes to: https://github.com/android/nowinandroid/
#
...
if grep -P "" /dev/null > /dev/null 2>&1; then
    ...
    GREP_COMMAND=grep
    echo "WARN: You may not have a version of 'grep' installed which supports Perl regular expressions."
    echo "WARN: On MacOS you can install one using Homebrew with the command: 'brew install grep'"
    #exit 1
fi
